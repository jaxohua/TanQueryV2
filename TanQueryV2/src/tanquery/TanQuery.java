...
public class TanQuery extends PApplet {
	...
	int calPoints = 0;
	boolean calibrated = true; //si es falso hace calibración
	

	PVector[] cal = new PVector[4];
	PVector[] dots = new PVector[4];

	int lineLength = 20; // Se utiliza para dibujar los puntos de calibracion
							// draw
	...
	// DBConnect conex = new DBConnect("localhost", 3306, "escuela", "root",
	// "ax");
	DBConnect conex;
	// DBConnect conex = new DBConnect("localhost", "3306", "escuela",
	// "tanquery", "$tanquery$");
	...
	public ArrayList<TuioObject> tuioObjectList;
	public ArrayList<Consulta> consulta; // Array List con tokens que se
											// encuentran
	...
	public void setup() {
		...
		myTextarea = cp5.addTextarea("txt").setPosition(510, 50)
				.setSize(500, 200).setFont(createFont("arial", 16))
				.setLineHeight(14).setColor(color(255))
				.setColorBackground(color(59, 131, 189))
				.setColorForeground(color(255, 0, 0));
		;

		// SE MANDA A LEER EL ARCHIVO CALIBRACIÓN
		// this.calPoints=0;
		// this.calibration();
		...
		comparation = Valores.fillComparison();
	...
	public void draw() {
		...
		// Pruebas.addToConsulta(this); Copia los Tokens hacia la lista de
		// Consulta
		...
		// this.arbolConsul=new ArbolConsulta();//crea de nuevo el árbol de
		// consulta
		...
		//	noFill();
			// ellipseMode(CENTER);
			// ellipse( dots[calPoints].x, dots[calPoints].y, lineLength * 2,
			// lineLength * 2 );
		...
		if (Busquedas.objectsInRange(this) > 0) { // si hay objetos dentro del
													// area de asignación de
													// valor
			fill(0, 200, 0);
			textFont(font, 14);
		...
		for (TuioObject objeto : tuioObjectList) { 
			// get a list of all objects identified by webcam
			...
			if (Busquedas.isinRange(objetoX, objetoY)) {
				...
				if (Valores.whatIs(objeto.getSymbolID()).equals("operator")) {
					if (Valores.onList(id, this.tokens)) {
						descrip = Valores.getDescOper(this.operadores, Valores
								.getToken(id, this.tokens).getValorDisp());
						Display.dispAyuda(this, descrip);
						// Log.addLineCsv(nombreLog, Integer.toString(id)
						// +","+Valores.whatIs(objeto.getSymbolID())+ ","
						// +Integer.toString(objetoX)+ ","+
						// Integer.toString(objetoY)+","+Valores.getToken(objeto.getSymbolID(),this.tokens).getValorDisp(),"asignación");
					}
				} else {
					this.myTextarea.hide();
				}
			...
			if (Busquedas.isInToken(tokens, id)
					&& Busquedas.isInTrabajo(objetoX, objetoY)) {// manda la
																	// posicion
																	// X,Y del
																	// Objeto
			...
			if (this.arbolConsul.getRaiz() != null) {
				...
				if (nodoEncontrado != null) {
					// frame.arbolConsul.removeNodo(nodoEncontrado); //Si
					// encuentra el nodo en el arbol, lo elimina
					if (token != null) {
	...
	public void addTuioObject(TuioObject tobj) {
		...
		tuioObjectList = tuioClient.getTuioObjectList();
		if (arbolConsul.getRaiz() != null) {
			raiz = arbolConsul.getRaiz().valor;
		}
		// si el árbol tiene raiz

		// tuioObjectList = tuioClient.getTuioObjectList();
		for (TuioObject objeto : tuioObjectList) {
			if (arbolConsul.getRaiz() != null) {
				// busca si existe el id dentro del árbol y lo recupera
				nodo = this.arbolConsul.getNodo(objeto.getSymbolID());
			}
			log.addLineCsv(this, nodo, "addTuioObject", raiz, objeto,momento);
		}
		momento++;
	...
	public void updateTuioObject(TuioObject tobj) {
		...
		for (TuioObject objeto : tuioObjectList) {
			...
			log.addLineCsv(this, nodo, "updateTuioObject", raiz, objeto,momento);
		}
		momento++;

		// Log.addLineCsv(logpath, tokens, "updateObject", tobj, width, height,
		// arbolConsul.getRaiz().valor, nodo,tuioObjectList);
	...
	public void removeTuioObject(TuioObject tobj) {
		...
		for (TuioObject objeto : tuioObjectList) {
			...
			log.addLineCsv(this, nodo, "removeTuioObject", raiz, objeto,momento);
		}
		momento++;

		// Log.addLineCsv(logpath, tokens, "removeObject", tobj, width, height,
		// arbolConsul.getRaiz().valor, nodo,tuioObjectList);
	...
	public void calibration() {
		...
		if (lines != null && lines.length == 8) {
			...
			calibrated = true;
			System.out
					.println("R  E  A  D      C  A  L  I  B  R  A  T  I  O  N  Done.");
		}
	}
	...
	public void drawC() {
		int lineLength = 10;
		this.fill(255);
		this.ellipse(dots[calPoints].x, dots[calPoints].y, lineLength * 2,
				lineLength * 2);
	...
	int corrected_x(TuioObject tobj) {
		if (tobj != null)
			return (int) ((a1 * tobj.getScreenX(width) + b1
					* tobj.getScreenY(height) + c1) / (a3
	...
	int corrected_y(TuioObject tobj) {
		if (tobj != null)
			return (int) ((this.a2 * tobj.getScreenX(width) + this.b2
					* tobj.getScreenY(height) + this.c2) / (this.a3
