...
public class Calibration_back {
	...
	public Calibration_back(TanQuery frame) {
		...
		dots[3] = new PVector(width - calibInset, height - calibInset); // bot
																		// right
		frame.text("Loading calibration file...", 800, 50);
		// if we already have some data, then we used the saved configuration
		String lines[] = frame.loadStrings("src/data/calibration.txt");
		if (lines != null ) {
	...
	public Calibration_back(){
		super();
	}
	public void mostrar(MyCalibrate my) {//Solo es de prueba para probar loadstrings()
		this.my=my;
		my.text("Loading calibration file...check in eclipse console.", 800, 50);
		// if we already have some data, then we used the saved configuration
	...
	public void escribir(){
		...
		try {
			...
			my.saveStrings("src/data/calibration2.txt", list);
...
public void setup() {
	...
	void getCalibrationPoint(int x, int y, TanQuery frame) {
		if (calibrated == false) {
			cal[calPoints++] = new PVector(x, y);

			if (calPoints == 4) {
				if (calibrate(frame) == 0)
					calibrated = true;
				else
					calPoints = 0;
			}
	...
	@SuppressWarnings("static-access")
	public int calibrate(TanQuery frame) {
		...
		frame.saveStrings("src/data/calibration.txt", frame.split(data, ';'));

		return 0;
	}

	public void draw(TanQuery frame) {
		int lineLength = 30;

		frame.stroke(255);
		frame.fill(255,0,0);
		//System.out.println("Valor de lineLen="+lineLength);
		frame.ellipse(dots[calPoints].x, dots[calPoints].y, lineLength * 2,lineLength * 2);
		frame.line(dots[calPoints].x - lineLength, dots[calPoints].y,dots[calPoints].x + lineLength, dots[calPoints].y);
		frame.line(dots[calPoints].x, dots[calPoints].y - lineLength,dots[calPoints].x, dots[calPoints].y + lineLength);
	}
